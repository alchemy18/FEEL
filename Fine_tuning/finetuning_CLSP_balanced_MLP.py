...
def run_training_once(df_train_full, df_test_full, feature_cols, label_col, ckpt_path, percentage,
    ...
    # Concatenate to create perfectly balanced training set
    df_train_small = pd.concat([df_0_small, df_1_small]).sample(frac=1.0, random_state=42)
    
    # Print final training data distribution
    # final_counts = df_train_small[label_col].value_counts().sort_index()
    # print(f"Final training data - Class 0: {final_counts[0]} samples")
    # print(f"Final training data - Class 1: {final_counts[1]} samples")
    # print(f"Final training data - Total: {len(df_train_small)} samples")
    # print(f"Final training data - Balance ratio: {final_counts[0]/final_counts[1]:.2f}")

    # Create datasets (removed remaining train dataset)
    ...
    if label_col == "arousal_category":
        text_class = [
            "The participant felt low energy and relaxed, with calm emotions like peacefulness, relaxed, neutral, boredom, and lack of interest.",
            "The participant felt a strong physical reaction, like a racing heart or tense body, and experienced high-energy emotions such as excitement, enthusiasm, surprise, anger, and nervousness.",
        ]
    else:
        text_class = [
            "The participant felt bad and was in a negative mood, with emotions like sadness, fear, depression, anger, stress, worry, hopelessness, and frustration.",
            "The participant experienced a positive mood characterized by emotions such as joy, gratitude, serenity, interest, hope, pride, amusement, inspiration, awe, happy and love."
        ]
...
def train_cocoop(dataset_name, csv_path, ckpt_path=None, feature_cols=None, label_col=None, percentage=None,
    ...
    model_name_1 = f"./{dataset_name}/{dataset_name}_{feature_name}_{label_col}_{percentage}_{oversampling_method}_run1.pt"
    model_name_2 = f"./{dataset_name}/{dataset_name}_{feature_name}_{label_col}_{percentage}_{oversampling_method}_run2.pt"
                ...
                for pct in percentages:
                    for bc in [4]:
                        for ep in [15]:
                            for lr in [5e-5]: 
                                for n_ctx in [16]:
                                    # for me in [8, 16, 32, 64]:
                                    for me in [32]:
                                        print(f"Label : {label}, Modality : {modality_name}, Percentage : {pct}, Meta-Net Hidden Dim : {me}")
