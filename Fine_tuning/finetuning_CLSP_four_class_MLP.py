...
class CoCoOpPromptLearner(nn.Module):
    def __init__(self, feature_dim=100, n_ctx=4, output_dim=768):
        super().__init__()
        self.n_ctx = n_ctx
        ...
        self.meta_net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(feature_dim // 16, output_dim)
        )

    def forward(self, ts_features):
        bias = self.meta_net(ts_features).unsqueeze(1)
        return self.ctx.unsqueeze(0) + bias

class CoCoOpCLSP(nn.Module):
    def __init__(self, feature_dim, n_classes=4, n_ctx=4):  # Changed to 4 classes
        super().__init__()
...
def train_cocoop(dataset_name, csv_path, ckpt_path=None, feature_cols=None, label_col=None, percentage=None,
                batch_size=4, epochs=300, lr=1e-5, device='cuda'):
    ...
    run_name_1 = f"Run 1"
    run_name_2 = f"Run 2"
    model_name_1 = f"./{dataset_name}/{dataset_name}_{feature_name}_{label_col}_{percentage}_run1.pt"
    model_name_2 = f"./{dataset_name}/{dataset_name}_{feature_name}_{label_col}_{percentage}_run2.pt"
...
def fourclassfinetunemlp(dataset_name, eda_path, ppg_path, com_path):
        ...
        for label in labels:
            ...
            for pct in percentages:
                model = train_cocoop(
                    ...
                    ckpt_path=f"{modality_data['ckpt'][label]}",
