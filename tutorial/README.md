...
| **transformers** | Utilized for the Contrastive Language Signal Pre-training (CLSP) text encoder and fine-tuning of the CLSP using two Meta-Net family (CNN, MLP). |
...
│   ├── visualize.py # for visualizating TSNE and UMAP and data distribuation
...
The framework uses 19 publicly available datasets. The details are given in the README.MD you can follow each dataset's usage policy to download them. 
You can extend the framework on other datasets, by following their usage policy. 
...
#### 3.1.1 Running Base experiements 
...
We recommend creating a tmux instance for each dataset and run the script individually following given steps. 
...
#### 3.1.2 Running CLSP FineTuning experiements 
CLSP fine-tuning takes on average 20 minutes of the GPU hours. We recommend creating tmux instance. 
...
#### 3.2.1 Running Base experiements 

We recommend creating a tmux instance for each dataset and run the script individually following this command. 
```bash
...
#### 3.2.2 Running CLSP FineTuning experiements 
We recommend creating tmux instance  for each dataset and run the script individually following this command. . 
...
You can create multiple grouping based upon your requiremnent, we have made three sets of grouping (Experimental Setting,  Device Type,  Labeling Method)
#### 3.3.1 Curating csv files for each group 
Please refer to the Data_Preparation.ipynb that has steps for curating csv files for the each group. 

#### 3.2.2 Running Experiments
Group_running.py file runs both base experiments and CLSP fine tuning experiments.
We recommend creating tmux instance  for each group and run the script individually following this command. . 
```bash
...
- To add a new model, write the new_model.py file and add to the script folder.
...
Fixes:
- Most of the time out of memory issue comes for the large datasets, for that run the eda_artifact.py file for that individual dataset on an individual core.
