...
def train_LDA_unified_emotion(X_train, y_train_emotion, X_test, y_test_emotion):
    """Train LDA model for unified emotion classification (4 classes)"""
    
    # For 4-class problem, we need at most 3 components (n_classes - 1)
    # LDA automatically determines the optimal number of components
    model_emotion_LDA = LinearDiscriminantAnalysis()
    model_emotion_LDA.fit(X_train, y_train_emotion)

    # Predict on the test set
    y_pred_emotion_LDA = model_emotion_LDA.predict(X_test)

    # Calculate metrics
    accuracy_emotion_LDA = accuracy_score(y_test_emotion, y_pred_emotion_LDA)
    balanced_acc_emotion_LDA = balanced_accuracy_score(y_test_emotion, y_pred_emotion_LDA)
    
    # For multiclass F1 score, use 'weighted' average
    f1_emotion = f1_score(y_test_emotion, y_pred_emotion_LDA, average='weighted')
    
    # R2 and MSE for multiclass
    r2_emotion = r2_score(y_test_emotion, y_pred_emotion_LDA)
    mse_emotion = mean_squared_error(y_test_emotion, y_pred_emotion_LDA)

    return accuracy_emotion_LDA, balanced_acc_emotion_LDA, r2_emotion, mse_emotion, f1_emotion

def UnifiedEmotion(PathFile, output_file=None):
    """
    Train LDA for unified emotion classification combining arousal and valence
    """
    ...
    results = []
    accuracy_emotion_LDA, balanced_acc_emotion_LDA, r2_emotion, mse_emotion, f1_emotion = {}, {}, {}, {}, {}
    ...
    for pi in tqdm(X_train['PID'].unique(), desc='Processing PID'):
        ...
        # Train model
        accuracy_emotion_LDA[pi], balanced_acc_emotion_LDA[pi], r2_emotion[pi], mse_emotion[pi], f1_emotion[pi] = train_LDA_unified_emotion(
        ...
        results.append({
            ...
            'Test_Accuracy': accuracy_emotion_LDA[pi],
            'Test_F1': f1_emotion[pi]
        })
    
    # Calculate average metrics
    acc_emotion = sum(accuracy_emotion_LDA.values()) / len(accuracy_emotion_LDA.values())
    f1_emotion_avg = sum(f1_emotion.values()) / len(f1_emotion.values())
    
    print(f"------ Average accuracy of LDA on unified emotion classification: {acc_emotion:.4f} ------")
    print(f"------ Average F1 score of LDA on unified emotion classification: {f1_emotion_avg:.4f} ------")
