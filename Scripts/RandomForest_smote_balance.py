...
def Arousal(PathFile,output_file=None):
    ...
    for pi in tqdm(X_train['PID'].unique(), desc='Processing PID'):
        ...
        try:
            ...
            train_x_balanced, train_arousal_balanced = smote.fit_resample(train_x, train_arousal)
        except ValueError as e:
            # If SMOTE fails (e.g., not enough samples), fall back to original data
            print(f"SMOTE failed for PID {pi}: {e}. Using original data.")
            train_x_balanced, train_arousal_balanced = train_x, train_arousal

        accuracy_arousal_RF[pi], balanced_acc_arousal_RF[pi], r2_a[pi], mse_a[pi], f1_a[pi] = train_random_forest_arousal(
            train_x_balanced, train_arousal_balanced, test_x, test_arousal)
...
def Valence(PathFile,output_file=None):
    ...
    for pi in tqdm(X_train['PID'].unique(), desc='Processing PID'):
        ...
        try:
            smote = SMOTE(random_state=seed)
            train_x_balanced, train_valence_balanced = smote.fit_resample(train_x, train_valence)
        except ValueError as e:
            # If SMOTE fails (e.g., not enough samples), fall back to original data
            print(f"SMOTE failed for PID {pi}: {e}. Using original data.")
            train_x_balanced, train_valence_balanced = train_x, train_valence

        accuracy_valence_RF[pi], balanced_acc_valence_RF[pi], r2_v[pi], mse_v[pi], f1_v[pi] = train_random_forest_valence(
            train_x_balanced, train_valence_balanced, test_x, test_valence)
