...
class Text_Encoder(nn.Module):
    ...
    def text_tokens(self, batch):
        ...
        for texts in batch:
            ...
            with torch.no_grad():
                model_output = self.model(**encoded_input)  # Fixed: was 'model', should be 'self.model'
...
class CLIPModel(nn.Module):
    ...
    def forward(self, batch, encoded_label1, encoded_label2):
        ...
        # Get embeddings
        eda_embeddings = self.eda_encoder.get_hidden_embedding(data_values)
        
        # Compute similarities
        if encoded_label1.dim() == 1:
            encoded_label1 = encoded_label1.unsqueeze(0)
        if encoded_label2.dim() == 1:
            encoded_label2 = encoded_label2.unsqueeze(0)
            
        # Compute dot products for similarity
        a = torch.sum(eda_embeddings * encoded_label1, dim=-1)
        b = torch.sum(eda_embeddings * encoded_label2, dim=-1)
        
        # Stack for binary classification output (batch_size, 2)
...
def measure_clsp_metrics(model, sample_batch, device):
    ...
    label2 = "Data of High and more arousal_category"
    ...
    # ============================================================
    # PER-INFERENCE: EDA Encoder + Matrix Multiplication
    # ============================================================
    print("\n[PER-INFERENCE - Each forward pass]")
    
    dummy_eda = sample_batch['data'].to(device)
    input_dim = dummy_eda.shape[1]
    batch_size = dummy_eda.shape[0]
    
    # EDA Encoder (MLP: input_dim -> 50 -> 100 -> output_dim)
    eda_encoder_flops = (
        2 * batch_size * input_dim * 50 +   # Linear1: input -> 50
        2 * batch_size * 50 * 100            # Linear2: 50 -> 100
    )
    
    print(f"  EDA Encoder: {eda_encoder_flops/1e6:.4f} MFLOPs, {eda_params/1e6:.4f}M params")
    
    # Matrix Multiplications (2x for similarity computation)
    eda_hidden_dim = 100
    ...
    inference_params = eda_params
    
    print(f"  → TOTAL PER-INFERENCE: {inference_flops/1e6:.4f} MFLOPs")
    print(f"  → INFERENCE PARAMS:    {inference_params/1e6:.4f} M")
    
    # ============================================================
    # ONE-TIME INFERENCE CALCULATION
    # ============================================================
    one_time_flops = preprocessing_flops + inference_flops
    one_time_params = preprocessing_params + inference_params
    ...
    print(f"  - Projection Head:   {projection_params/1e6:.4f} M")
    print(f"  - EDA Encoder:       {eda_params/1e6:.4f} M")
    print(f"FLOPs: {one_time_flops/1e6:.2f} MFLOPs")
    print(f"  - Preprocessing:     {preprocessing_flops/1e6:.2f} MFLOPs (Text + Projection)")
    print(f"  - Inference:         {inference_flops/1e6:.4f} MFLOPs (EDA Encoder)")
    
    print(f"\n--- SUBSEQUENT INFERENCES (2nd+ predictions) ---")
    print(f"Parameters Used: {inference_params/1e6:.4f} M (EDA Encoder only)")
    print(f"FLOPs: {inference_flops/1e6:.4f} MFLOPs per prediction")
    print(f"  - EDA Encoder:       {eda_encoder_flops/1e6:.4f} MFLOPs")
    print(f"  - Matrix Mult:       {matmul_flops/1e6:.6f} MFLOPs")
    
    print(f"\nFor N predictions: {preprocessing_flops/1e6:.2f} + (N × {inference_flops/1e6:.4f}) MFLOPs")
    print("="*60 + "\n")
    
    return {
        'whole_pipeline_mflops': one_time_flops / 1e6,
        ...
        'inference_params_m': inference_params / 1e6,
...
if __name__ == "__main__":
    ...
    # Initialize model
    model = CLIPModel(mlp_input_dim=num_features, mlp_output_dim=1, device=device)
    ...
    if results['status'] == 'success':
        ...
        # Latency
        print(f"{'Preprocessing Latency (one-time):':<50} {results['preprocessing_latency_ms']:>17.4f} ms")
        print(f"{'Per-Inference Latency:':<50} {results['inference_latency_ms']:>17.4f} ms")
        print(f"{'Total Pipeline Latency (1st prediction):':<50} {results['total_pipeline_latency_ms']:>17.4f} ms")
        
        # FLOPs
        print()
        print(f"{'Whole Pipeline FLOPs (1st prediction):':<50} {results['whole_pipeline_mflops']:>17.2f} MFLOPs")
        print(f"{'  - Preprocessing (Text + Projection):':<50} {results['preprocessing_mflops']:>17.2f} MFLOPs")
        print(f"{'  - Inference (EDA Encoder):':<50} {results['per_inference_mflops']:>17.4f} MFLOPs")
        print(f"{'Per-Inference FLOPs (2nd+ predictions):':<50} {results['per_inference_mflops']:>17.4f} MFLOPs")
        
        # Parameters
        print()
        print(f"{'--- PARAMETER BREAKDOWN ---':<50}")
        print(f"{'Total Model Parameters:':<50} {results['total_params_m']:>17.4f} M")
        print(f"{'  - Text Encoder:':<50} {results['text_encoder_params_m']:>17.4f} M")
        print(f"{'  - Projection Head:':<50} {results['projection_params_m']:>17.4f} M")
        print(f"{'  - EDA Encoder:':<50} {results['eda_encoder_params_m']:>17.4f} M")
        
        print()
        print(f"{'One-Time Inference Params (1st prediction):':<50} {results['one_time_params_m']:>17.4f} M")
        print(f"{'  - Text Encoder:':<50} {results['text_encoder_params_m']:>17.4f} M")
        print(f"{'  - Projection Head:':<50} {results['projection_params_m']:>17.4f} M")
        print(f"{'  - EDA Encoder:':<50} {results['eda_encoder_params_m']:>17.4f} M")
        
        print()
        print(f"{'Subsequent Inference Params (2nd+):':<50} {results['inference_params_m']:>17.4f} M")
        print(f"{'  (EDA Encoder only)':<50}")
